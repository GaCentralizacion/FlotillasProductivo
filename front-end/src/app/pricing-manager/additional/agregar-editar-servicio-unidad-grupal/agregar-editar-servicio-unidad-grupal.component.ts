...
@Component({
  selector: 'app-agregar-editar-servicio-unidad-grupal',
  templateUrl: './agregar-editar-servicio-unidad-grupal.component.html',
  styleUrls: ['./agregar-editar-servicio-unidad-grupal.component.scss']
})
export class AgregarEditarServicioUnidadGrupalComponent implements OnInit {
  ...
  serviciosUnidadSelectedServ: ServicioUnidad[] = [];
  ...
  ngOnInit() {
    if (this.idGrupoUnidad != undefined) {
      ...
      this.pricingService.getAdicionalesGestionbyGrupal(this.cotizacion.idCotizacion,this.idGrupoUnidad).subscribe((adicionales: any) => { 
        ...
        //OCT99 GESTION
        this.grupoSelected.serviciosUnidadSinPaquete = adicionales[2];
  ...
  buscarServiciosUnidad() {
    this.serviceUnit.getServicios(this.cotizacion.idSucursal, this.catalogoSelected.idUnidadBpro, this.anioSelected)
      .subscribe((data: ServicioUnidad[]) => {
        ...
        this.serviciosUnidadSelectedServ = [];
        if (this.grupoSelected != undefined) {
          this.allServiciosUnidad.map(su => {
            ...
            if (su.isSelected) {
              ...
              this.serviciosUnidadSelectedServ.push(JSON.parse(JSON.stringify(su))); // chk guardo los mov originales seleccionado previamente
            }
          });
        }
      });
  ...
  guardarServiciosUnidad(tipoMovimiento) {
    ...
    if (this.modalGestoria) {
      const serviciosUnidadPorGuardar:ServicioUnidadSinPaqueteUnidad[] = [];
      if(this.serviciosUnidadSelectedServ.length > 0){
        let nuevo= this.serviciosUnidadSelected.filter( it=> {
          let objecto = this.serviciosUnidadSelectedServ.find(itSelects => itSelects.idServicioUnidad === it.idServicioUnidad);
      ...
      this.pricingService.saveCotizacionUnidadServicioGrupal(this.cotizacion.idCotizacion,
