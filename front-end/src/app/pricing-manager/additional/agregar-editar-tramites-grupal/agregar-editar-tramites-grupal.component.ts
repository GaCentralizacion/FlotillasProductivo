...
import { Cotizacion, TramiteSinPaquete, TramiteUnidad, Tramite, Subtramite, ProveedorAdicional, GrupoUnidades, PaqueteTramite, ProductoCFDI, TramiteUnidadFac } from '../../../models';
...
export class AgregarEditarTramitesGrupalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.tramitesService.getTramites(this.cotizacion.idMarca, this.cotizacion.idSucursal,this.cotizacion.idDireccionFlotillas).subscribe((tramites: Tramite[]) => {
  ...
  tramiteOnChange(tramite: Tramite) {
    this.formProducto.controls.checkProducto.setValue(false);
    ...
    if (tramite != undefined) {
      this.tramitesService.getSubtramites(this.cotizacion.idMarca,
        ...
        this.tramitesService.getCatalogoDatosFac(this.tramiteSelected.idTramite,'Tramite').subscribe((ProductoCFDIs: ProductoCFDI[]) => {
  ...
  visualizaComboProductos(){
    ...
    if(this.formProducto.value.checkProducto == true){
      this.banderaCombo = 1
    }else{
      this.ProductoCFDIs = [];
      this.banderaCombo = 0
  ...
  guardarTramites(tipoMovimiento,tramiteValida) {
    ...
    if(this.banderaCombo==1 && this.bandera ==1){
      if(this.productoCFDIsSelected == undefined ){
        this.toastrService.warning(`Debe seleccionar un tipo de produto para continuar.`, `Validación de Tramite`);
        return true;
      }
    }
    ...
    if (this.modalGestoria) {
      ...
      */
      if(this.banderaCombo==1 && this.bandera ==1){
        const tramitesFac = [];
        const tramiteUnicoFac = {
          idCotizacion: this.cotizacion.idCotizacion,
          idGrupoUnidad: (this.idGrupoUnidad != undefined)? this.idGrupoUnidad: -1,
          idDetalleUnidad: 1,
          idTramite: this.tramiteSelected.idTramite.toString(),
          idSubtramite: this.subtramiteSelected.idSubtramite.toString(),
          precio: this.precioEditado,//this.proveedoresSelected[0].precio,
        ...
        tramitesFac.push(tramiteUnicoFac);

        this.tramitesService.guardaDatosFacTramite(tramitesFac).subscribe(() => {
        });
      }
        ...
        this.idGrupoUnidad, this.idDetalleUnidad, tramites).subscribe(() => {
          if(this.guardaTramite)
            ...
            if(this.banderaCombo==1 && this.bandera ==1){
              ...
              this.tramitesService.guardaDatosFacTramiteMov(tramitesFac).subscribe(() => {
              });
            }
