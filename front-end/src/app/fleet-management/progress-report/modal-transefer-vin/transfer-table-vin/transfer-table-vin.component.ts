...
@Component({
  selector: 'app-transfer-table-vin',
  templateUrl: './transfer-table-vin.component.html',
  styleUrls: ['./transfer-table-vin.component.scss'],
  providers: [PricingManagerService]
})
export class TransferTableVinComponent implements OnInit {

  @ViewChild('transferTableComponent1') childTable: TransferTableVinComponent;
  ...
  onSelect(event) {
    console.log('table mov trans post ', event);
  ...
  async deleteChildren(rows) {
    rows.forEach(async row => {
      await this.trasladosService.eliminaTrasladoPosteriorMov(row.idCotizacionTraslado).toPromise();
    });
  }
