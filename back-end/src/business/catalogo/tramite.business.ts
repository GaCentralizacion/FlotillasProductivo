...
export class TramiteBusiness {
    getTramites(idMarca: string, idSucursal: number, idDireccionFlotillas: string): Promise<Tramite[]> {
        return new Promise<Tramite[]>(async (resolve, reject) => {
            const connection = getConnection();
            await connection.query(`exec sp_Flotillas_tramites @Marca='${idMarca}', @Sucursal=${idSucursal}, @idDireccionFlotillas='${idDireccionFlotillas}'`).then((resultadosTramite: any[]) => {
