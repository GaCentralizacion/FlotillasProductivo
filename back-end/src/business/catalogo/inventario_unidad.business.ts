...
export class InventarioUnidadBusiness {
    ...
    getCatalogoDatosFac(idFac: string, idTipo: string): Promise<any> {
        return new Promise<any>(async (resolve, reject) => {
            const connection = getConnection();
            await connection.transaction(async (manager) => {
                await connection.query(`exec SEL_VALIDADATOS_FAC_SP @idFac = '${idFac}', @idTipo = '${idTipo}'`)
                    .then((resp: any[]) => {
                        if (!resp.length) {
                            reject({ status: 404, error: `No se pudo recuperar los datos` });
                        }
    ...
    guardaDatosFacTramite(idCotizacion: string, idGrupoUnidad: number, idDetalleUnidad: number, idTramite: string,
                          idSubtramite: string, precio: number, datosFac: string, idUsuario: number): Promise<any> {
        return new Promise<any>(async (resolve, reject) => {
            const connection = getConnection();
            await connection.transaction(async (manager) => {
                await connection.query(`exec sp_Flotillas_GuardaDatosFacTramite @idCotizacion = '${idCotizacion}'
    ...
    guardaDatosFacTramiteMov(idCotizacion: string, idGrupoUnidad: number, idDetalleUnidad: number, idTramite: string,
                             idSubtramite: string, precio: number, datosFac: string, idUsuario: number): Promise<any> {
        return new Promise<any>(async (resolve, reject) => {
            const connection = getConnection();
            await connection.transaction(async (manager) => {
                await connection.query(`exec sp_Flotillas_GuardaDatosFacTramiteMov @idCotizacion = '${idCotizacion}'
                , @idGrupoUnidad = '${idGrupoUnidad}', @idDetalleUnidad = '${idDetalleUnidad}', @idTramite = '${idTramite}'
                , @idSubTramite = '${idSubtramite}', @precio = '${precio}', @datosFac = '${datosFac}', @idUsuario = '${idUsuario}'`)
                    .then((resp: any[]) => {
    ...
    guardaDatosFacTraslado(idCotizacionTraslado: number, idCotizacion: string, idGrupoUnidad: number, idTraslado: number,
                           datosFac: string, idUsuario: number): Promise<any> {
        return new Promise<any>(async (resolve, reject) => {
            const connection = getConnection();
            await connection.transaction(async (manager) => {
                await connection.query(`exec sp_Flotillas_GuardaDatosFacTraslado @idCotizacionTraslado = '${idCotizacionTraslado}'
